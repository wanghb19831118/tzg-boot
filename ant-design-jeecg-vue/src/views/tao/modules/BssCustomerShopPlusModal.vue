    ...
    <a-spin :spinning="confirmLoading">
      ...
      <a-tabs v-model="activeKey" @change="handleChangeTabs">
        <a-tab-pane tab="服务账号" :key="refKeys[0]" :forceRender="true">
          <j-editable-table
            :ref="refKeys[0]"
            :loading="bssShopServiceAccountTable.loading"
            :columns="bssShopServiceAccountTable.columns"
            :dataSource="bssShopServiceAccountTable.dataSource"
            :maxHeight="300"
            :rowNumber="true"
            :rowSelection="true"
            :actionButton="true"/>
        ...
        <a-tab-pane tab="店铺指标" :key="refKeys[1]" :forceRender="true">
          <j-editable-table
            :ref="refKeys[1]"
            :loading="bssShopKpiTable.loading"
            :columns="bssShopKpiTable.columns"
            :dataSource="bssShopKpiTable.dataSource"
            :maxHeight="300"
            :rowNumber="true"
            :rowSelection="true"
            :actionButton="true"/>
  ...
  export default {
    ...
    data() {
      return {
        ...
        refKeys: ['bssShopServiceAccount', 'bssShopKpi'],
        activeKey: 'bssShopServiceAccount',
        statusoptions: [],//状态  0为无效 1为有效
        platformoptions: [],//平台数据
        typeoptions: [],//类型 0主权限账户1一般客服账户
        // 服务账号
        bssShopServiceAccountTable: {
          loading: false,
          dataSource: [],
          columns: [
            {
        ...
        url: {
          add: '/tao/bssCustomerShop/add',
          edit: '/tao/bssCustomerShop/edit',
          type: '/tao/bssCustomerShop/edit',
          bssShopServiceAccount: {
            list: '/tao/bssCustomerShop/queryBssShopServiceAccountByMainId'
          },
          bssShopKpi: {
            list: '/tao/bssCustomerShop/queryBssShopKpiByMainId'
          }
        }
      }
    },
    ...
    methods: {
      ...
      initDictConfig() {
        ...
        initDictOptions('status').then((res) => {
          if (res.success) {
            ...
            // that.$set(that.bssShopServiceAccountTable.columns[1], 'options', res.result)
            // that.bssShopServiceAccountTable= Object.assign({}, that.bssShopServiceAccountTable)
            // that.$forceUpdate();
      ...
      editAfter() {
        ...
        if (this.model.id) {
          ...
          // this.$set(this.bssShopServiceAccountTable.columns[1], 'options', this.statusoptions)
          this.bssShopServiceAccountTable.columns[1].options = this.statusoptions
          this.bssShopServiceAccountTable = Object.assign({}, this.bssShopServiceAccountTable)
          this.requestSubTableData(this.url.bssShopServiceAccount.list, params, this.bssShopServiceAccountTable)
          this.requestSubTableData(this.url.bssShopKpi.list, params, this.bssShopKpiTable)
        }
      },
      ...
      classifyIntoFormData(allValues) {
        ...
        return {
          ...main, // 展开
          bssShopServiceAccountList: allValues.tablesValue[0].values,
          bssShopKpiList: allValues.tablesValue[1].values
        }
      }
    }
  }
